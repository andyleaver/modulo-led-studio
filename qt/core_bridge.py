...
from app.eras.era_enforce import enforce_project_against_era, EraViolation
from app.eras.era_history import get_era
...
def _ensure_default_strip60_in_project(p: dict) -> tuple[dict, bool]:
    ...
    try:
        ...
        # Back-compat: legacy 'matrix' should be treated as canonical 'cells'.
        if shape == "matrix":
            shape = "cells"
            lay["shape"] = "cells"
            if str(lay.get("type") or "").strip().lower() == "matrix":
                lay["type"] = "cells"

        # If matrix dimensions are present, prefer 'cells' unless explicitly strip.
        try:
            mw = int(lay.get("matrix_w") or lay.get("w") or lay.get("width") or 0)
            mh = int(lay.get("matrix_h") or lay.get("h") or lay.get("height") or 0)
            if mw > 1 and mh > 1 and shape in ("", "strip"):
                shape = "cells"
                lay["shape"] = "cells"
                lay.setdefault("type", "cells")
        except Exception:
            pass
...
class CoreBridge:
        ...
        try:
            if hasattr(self, 'pm') and self.pm is not None and hasattr(self.pm, 'get'):
                ...
                if isinstance(p0, dict) and p0:
                    ...
                    # Era system: ensure an era_id exists in the project (idempotent).
                    try:
                        p4 = ensure_era_in_project(self._project)
                        if isinstance(p4, dict) and p4 is not self._project:
                            self._project = p4
                            try:
                                if hasattr(self, 'pm') and self.pm is not None and hasattr(self.pm, 'set'):
                                    self.pm.set(p4)
                    ...
                    # Era templates: on first-run (era onboarding incomplete), replace the project
                    # with the current era's template so users immediately see era-accurate capability.
                    try:
                        ui0 = (self._project.get('ui') or {}) if isinstance(self._project, dict) else {}
                        if isinstance(ui0, dict):
                            era_complete0 = bool(ui0.get('era_complete', False))
                            if not era_complete0 and not bool(ui0.get('era_template_applied', False)):
                                era_id0 = str(ui0.get('era_id') or '')
                                tpl = get_era_template_project(era_id0)
    ...
    def project(self, value):
        ...
        # Era system: ensure era_id exists (idempotent)
        try:
        ...
        try:
            ...
            pass
        # Era system hard-fail: project must not exceed historical gates.
        try:
    ...
    def _rebuild_full_preview_engine(self):
        ...
        try:
            ...
            # Keep a live reference to the dict used to build the engine for diagnostics.
            try:
                self._full_preview_engine.project_data = clean_proj
            except Exception:
                pass

            # Mark preview as needing a sync-to-project pass after rebuild.
            try:
                self._preview_dirty = True
            except Exception:
                pass
            ...
            if getattr(lay, "shape", "strip") == "cells":
                ...
                self._full_preview_geom = build_cells_geom(
                    mw, mh, cell,
                    serpentine=bool(layout_dict.get("serpentine", False)),
    ...
    def rebuild_preview(self, reason: str = "project_mutated") -> None:
        ...
        """
        try:
            # If layout changed (strip <-> cells, dimensions, serpentine, rotation, etc.),
            # the preview geometry must be rebuilt. A simple project sync is not enough.
            lay = (self.project or {}).get("layout") or {}
            if not isinstance(lay, dict):
                lay = {}
            sig = (
                str(lay.get("shape") or "strip").lower().strip(),
                int(lay.get("num_leds") or lay.get("count") or 0),
                int(lay.get("matrix_w") or 0),
                int(lay.get("matrix_h") or 0),
                bool(lay.get("serpentine", False)),
                bool(lay.get("flip_x", False)),
                bool(lay.get("flip_y", False)),
                int(lay.get("rotate") or 0),
                float(lay.get("cell_size") or lay.get("cell") or 0.0),
            )
            if sig != getattr(self, "_last_layout_sig", None):
                self._last_layout_sig = sig
                # Full rebuild ensures engine + geometry + caches match the new layout.
                self._rebuild_full_preview_engine()
            else:
