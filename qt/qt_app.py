...
# action mapping
        action: dict = {}
        if actk in ("set_var_num", "add_var_num"):
            action["kind"] = "set_var" if actk == "set_var_num" else "add_var"
...
class LayerStackPanel(QtWidgets.QWidget):
    ...
    def _commit(self, p2):
        ...
        try:
            if hasattr(self.app_core, 'sync_preview_engine_from_project_data'):
...
class ControlsPanel(QtWidgets.QWidget):
    ...
    def _layout_changed(self, idx: int):
        try:
            p = self.app_core.project or {}
            lay = dict(p.get("layout") or {})
            lay["shape"] = "strip" if int(idx) == 0 else "cells"
            if lay["shape"] != "strip":
                lay.setdefault("matrix_w", int(self.mw_spin.value()) if hasattr(self, "mw_spin") else 16)
                lay.setdefault("matrix_h", int(self.mh_spin.value()) if hasattr(self, "mh_spin") else 16)
            p2 = dict(p)
            p2["layout"] = lay
            self.app_core.project = p2
            try:
...
class QtMainWindow(QtWidgets.QMainWindow):
    def __init__(self, app_core):
        ...
        # : Qt autosave (minimal). Persist project to out/autosave_project.json
        # on a throttled timer so next launch resumes your last state.
        self._autosave_last_rev = -1
        self._autosave_last_t = 0.0
        try:
            self._autosave_timer = QtCore.QTimer(self)
            self._autosave_timer.setInterval(5000)  # 5s cadence (throttled below)
    ...
    def closeEvent(self, event):
        # One last autosave on exit (best-effort), then proceed with normal close.
        try:
            self._autosave_tick()
        except Exception:
            pass
