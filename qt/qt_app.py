...
class LayerStackPanel(QtWidgets.QWidget):
    ...
    def _add_layer(self):
        p, layers = self._project_layers()
        # Era gating: limit layer count in early eras.
        try:
            if hasattr(self, "app_core") and self.app_core is not None and hasattr(self.app_core, "get_era_gates"):
                gates = self.app_core.get_era_gates()
                max_layers = int(getattr(gates, "max_layers", 99))
                if len(layers) >= max_layers:
                    try:
                        QtWidgets.QMessageBox.information(self, "Era Locked", f"This era is limited to {max_layers} layer(s).")
                    except Exception:
                        pass
                    return
        except Exception:
    ...
    def _run_audit(self, *, include_audio: bool=False) -> str:
        ...
        try:
            for k in keys:
                ...
                # Force full capabilities during audit (do not let era gating block preview rebuilds)
                try:
                    ui = p.get('ui') if isinstance(p.get('ui'), dict) else {}
                    ui['era_complete'] = True
                    ui['era_id'] = 'era_now'
                    p['ui'] = ui
                except Exception:
                    pass
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        _p = getattr(self.app_core, 'project', None)
                        if isinstance(_p, dict):
                            if _ui_backup is not None:
                                _p['ui'] = _ui_backup
                    except Exception:
                        pass
                # Health check runs against the current project, which may be a diagnostics/showcase project.
                # If onboarding has been completed previously, historical era gating should not flag the audit.
                # Force-open era gates temporarily for the audit and restore afterwards.
                _ui_backup = None
                try:
                    _p = getattr(self.app_core, 'project', None)
                    if isinstance(_p, dict):
                        ...
                        if isinstance(_ui, dict):
                            ...
                            _ui_backup = None
                            _p['ui'] = {}
                        _p['ui']['era_complete'] = True
                        _p['ui']['era_id'] = 'era_now'
                except Exception:
                    _ui_backup = None
...
# --- end UI / Preview init diagnostics ---
            ...
            # --- ERA INTEGRITY (onboarding + history contracts) ---
            try:
                from app.eras.era_history import get_eras
                eras = get_eras()
                ids = [getattr(e, "era_id", None) for e in eras]
                ids = [x for x in ids if isinstance(x, str)]
                lines.append("== ERA INTEGRITY ==")
                lines.append(f"eras.total: {len(ids)}")
                dups = sorted({x for x in ids if ids.count(x) > 1})
                lines.append(f"eras.duplicate_ids: {dups if dups else 0}")
                lines.append(f"eras.has_era_now: {'era_now' in ids}")
                lines.append(f"eras.has_usage_plateau: {'era_usage_plateau' in ids}")
                try:
                    from qt.era_panel import EraPanel as _EraPanel
                    lines.append(f"ui.EraPanel._on_verify: {hasattr(_EraPanel, '_on_verify')}")
                    lines.append(f"ui.EraPanel._on_wb_changed: {hasattr(_EraPanel, '_on_wb_changed')}")
                except Exception as e:
                    lines.append(f"ui.EraPanel.import: FAIL ({type(e).__name__}: {e})")
                lines.append("")
            except Exception as e:
                lines.append("== ERA INTEGRITY ==")
                lines.append(f"import/get_eras: FAIL ({type(e).__name__}: {e})")
                lines.append("")
            # --- ENGINE PRIMITIVES (audited import check) ---
            try:
                lines.append("== Engine Primitives ==")
                mods = [
                    ("runtime.particles_v1", "ParticleSystemV1"),
                    ("runtime.particle_render_v1", "render_particlesystem_to_leds_v1"),
                    ("runtime.vector_fields_v1", "VectorField"),
                    ("runtime.noise_v2", "Noise2D"),
                    ("runtime.buffers_v1", "ScalarBufferV1"),
                    ("runtime.buffer_advection_v1", "advect_scalar_buffer_v1"),
                    ("runtime.buffer_render_v1", "render_scalar_buffer_to_leds_v1"),
                    ("runtime.influence_maps_v1", "deposit_points_scalar_v1"),
                    ("runtime.integrators_v1", "euler_step_entities"),
                    ("runtime.constraints_v1", "apply_constraints"),
                    ("runtime.particle_pairs_v1", "count_pairs_within_radius_v1"),
                    ("runtime.force_particles_core_v1", "integrate_point_forces_v1"),
                    ("runtime.system_scheduler_v1", "SystemSchedulerV1"),
                    ("runtime.extensions_v1", "register_system"),
                ]
                ok = 0
                for mod, sym in mods:
                    try:
                        ...
                        getattr(m, sym)
                        lines.append(f"- {mod}:{sym} [OK]")
                        ok += 1
                    except Exception as _ie:
                        lines.append(f"- {mod}:{sym} [MISSING] — {type(_ie).__name__}: {_ie}")
                lines.append(f"count_ok: {ok}/{len(mods)}")
                ...
                try:
                    ...
                    sched = getattr(eng, "systems", None)
                    snap = sched.snapshot() if sched is not None and hasattr(sched, "snapshot") else None
                    if isinstance(snap, dict):
                        order = snap.get("order") or []
                        errs = snap.get("errors") or []
                        lines.append("== System Scheduler Snapshot ==")
                        lines.append(f"systems: {len(order)}")
                        if order:
                            lines.append("order: " + " -> ".join([str(x) for x in order[:24]] + (["…"] if len(order) > 24 else [])))
                        lines.append(f"errors: {len(errs)}")
                except Exception:
                    pass
                lines.append("")
...
class ControlsPanel(QtWidgets.QWidget):
    ...
    def __init__(self, app_core, on_layout_changed, on_matrix_zoom_changed=None, on_matrix_jump=None, on_matrix_fit=None):
        ...
        # The controls column can grow tall (especially on small screens).
        # Wrap the entire left-side controls area in a scroll view so *all*
        ...
        outer = QtWidgets.QVBoxLayout(self._content)
        outer.setContentsMargins(8, 8, 8, 8)
        outer.setSpacing(8)
        ...
        # Tabs for controls/panels
        # NOTE: Era onboarding is handled by a dedicated full-screen window at boot.
        ...
        # Era gating: Operators unlock later.
        try:
            gates = self.app_core.get_era_gates() if hasattr(self.app_core, 'get_era_gates') else None
            if gates is None or bool(getattr(gates, 'allow_operators', True)):
                self.tabs.addTab(_wrap_tab_widget(self.operators_panel), 'Operators')
            else:
                self.operators_panel.setEnabled(False)
                self.tabs.addTab(_wrap_tab_widget(self.operators_panel), 'Operators (Locked)')
        except Exception:
            self.tabs.addTab(_wrap_tab_widget(self.operators_panel), 'Operators')

        # Quick diagnostics for effect coverage/regressions
        # NOTE: Diagnostics is intentionally the LAST tab in the main UI so it doesn't interrupt the authoring flow.
        ...
        # Era gating: Rules unlock later.
        try:
            gates = self.app_core.get_era_gates() if hasattr(self.app_core, 'get_era_gates') else None
            if gates is None or bool(getattr(gates, 'allow_rules', True)):
                self.tabs.addTab(_wrap_tab_widget(self.rules_panel), 'Rules')
            else:
                self.rules_panel.setEnabled(False)
                self.tabs.addTab(_wrap_tab_widget(self.rules_panel), 'Rules (Locked)')
        except Exception:
            self.tabs.addTab(_wrap_tab_widget(self.rules_panel), 'Rules')
        ...
        # Era gating: Export unlocks in later eras.
        try:
            gates = self.app_core.get_era_gates() if hasattr(self.app_core, "get_era_gates") else None
            if gates is None or bool(getattr(gates, "allow_export", True)):
                self.tabs.addTab(_wrap_tab_widget(self.export_panel), "Export")
            else:
                self.export_panel.setEnabled(False)
                self.tabs.addTab(_wrap_tab_widget(self.export_panel), "Export (Locked)")
        except Exception:
            self.tabs.addTab(_wrap_tab_widget(self.export_panel), "Export")
...
class QtMainWindow(QtWidgets.QMainWindow):
    def __init__(self, app_core):
        ...
        try:
            self.matrix_preview.setMinimumWidth(220)
        except Exception:
            pass
        ...
        # Default stretch/size policy (first run): favor controls so capability tabs
        # are immediately discoverable; keep preview at a modest width.
        # Saved splitter sizes (per-layout) will override this.
        self.body_split.setStretchFactor(0, 1)
        self.body_split.setStretchFactor(1, 0)

        try:
            # Controls wide, preview narrow (but still usable).
            self.body_split.setSizes([900, 280])
        except Exception:
            pass

        # Restore prior splitter sizing if available.
        # Fix38: persist separate splitter sizes for strip vs matrix layouts so
        # switching layouts doesn't destroy the user's chosen panel proportions.
        try:
            st = QtCore.QSettings('Modulo', 'ModuloLEDStudio')
            ...
            def _coerce_sizes(v):
                ...
                try:
                    ...
                    try:
                        # Some Qt bindings return a QVariant-like string.
                        if isinstance(v, str):
            ...
            legacy = _coerce_sizes(st.value('ui/body_split_sizes', None))
            strip_sizes = _coerce_sizes(st.value('ui/body_split_sizes_strip', None))
            matrix_sizes = _coerce_sizes(st.value('ui/body_split_sizes_matrix', None))

            # Backward compatibility: if only legacy exists, seed both modes.
            if legacy and (not strip_sizes):
                strip_sizes = legacy
            if legacy and (not matrix_sizes):
                matrix_sizes = legacy
            ...
            try:
                ...
                shape = "strip"
            self._layout_mode = "strip" if shape == "strip" else "matrix"

            use = strip_sizes if self._layout_mode == "strip" else matrix_sizes
            if use and len(use) >= 2:
                self.body_split.setSizes(use[:2])
        except Exception:
            # fall back to defaults
            try:
                self._layout_mode = "strip"
            except Exception:
                pass
        ...
        # Fix38: restore main window geometry/state if previously saved.
        # (run_qt() will avoid forcing a resize when saved geometry exists.)
        try:
            st = QtCore.QSettings('Modulo', 'ModuloLEDStudio')
            g = st.value('ui/main_geometry', None)
            if g:
                try:
                    self.restoreGeometry(g)
                except Exception:
                    pass
            s = st.value('ui/main_state', None)
            if s:
                try:
                    self.restoreState(s)
                except Exception:
    ...
    def closeEvent(self, event):
        # Persist UI sizing/state (does not touch project data).
        try:
            st = QtCore.QSettings('Modulo', 'ModuloLEDStudio')

            # Save main window geometry/state.
            try:
                st.setValue('ui/main_geometry', self.saveGeometry())
            except Exception:
                pass
            try:
                st.setValue('ui/main_state', self.saveState())
            except Exception:
                pass

            # Save splitter sizes per layout mode.
            try:
            ...
            try:
                if mode == "strip":
                    st.setValue('ui/body_split_sizes_strip', self.body_split.sizes())
                elif mode == "matrix":
                    st.setValue('ui/body_split_sizes_matrix', self.body_split.sizes())
                else:
                    # legacy fallback
                    st.setValue('ui/body_split_sizes', self.body_split.sizes())
            except Exception:
                pass
        except Exception:
            pass
    ...
    def _on_layout_changed(self):
        ...
        new_mode = "strip" if shape == "strip" else "matrix"

        # Persist the current mode's splitter sizes before switching.
        try:
            cur_mode = getattr(self, "_layout_mode", None)
            if cur_mode in ("strip", "matrix") and cur_mode != new_mode:
                st = QtCore.QSettings('Modulo', 'ModuloLEDStudio')
                try:
                    if cur_mode == "strip":
                        st.setValue('ui/body_split_sizes_strip', self.body_split.sizes())
        ...
        # Restore splitter sizes for the active mode (or fall back to sensible defaults).
        try:
            st = QtCore.QSettings('Modulo', 'ModuloLEDStudio')
            key = 'ui/body_split_sizes_strip' if new_mode == "strip" else 'ui/body_split_sizes_matrix'
            sizes = st.value(key, None)
            if sizes:
                try:
            ...
            if sizes and len(sizes) >= 2:
                self.body_split.setSizes(sizes[:2])
            else:
                # Defaults: strip wants controls wide, matrix wants preview wide.
                if new_mode == "strip":
...
def run_qt(app_core) -> None:
    ...
    # --- Era onboarding mode ---
    # If the user has not completed the era journey, boot into full-screen Era mode
    # and do NOT build the main app window.
    try:
        era_done = bool(getattr(app_core, 'is_era_complete', lambda: False)())
    except Exception:
        era_done = False

    if not era_done:
        era_win = EraOnboardingWindow(app_core)
        era_win.showMaximized()

        def _enter_full_modulo():
            try:
                era_win.close()
            except Exception:
                pass
            ...
            try:
                ...
                pass
            # Fix38: don't override saved window geometry.
            try:
                st = QtCore.QSettings('Modulo', 'ModuloLEDStudio')
                has_geo = bool(st.value('ui/main_geometry', None))
            except Exception:
                has_geo = False
            if not has_geo:
                win.resize(1180, 640)
        ...
        era_win.completed.connect(_enter_full_modulo)
        app.exec()
        return

    # Era already completed -> normal app boot
    win = QtMainWindow(app_core)
...
def _wrap_tab_widget(inner):
    ...
    # Ensure tab contents remain reachable on small screens.
    # We wrap *every* tab panel inside a scroll area, and also place the panel
    # in a simple container so its layout stays top-aligned and scroll bars
    # appear reliably even when the inner widget uses layouts that would
    # otherwise expand indefinitely.
    ...
    # Encourage sane sizing: let the scroll area control the viewport.
    try:
        inner.setSizePolicy(QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Maximum)
    except Exception:
        try:
            inner.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Maximum)
        except Exception:
            pass
    try:
