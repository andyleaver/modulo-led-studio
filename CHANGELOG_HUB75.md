## FIX38 (Window geometry + per-layout splitter persistence)
- Persist and restore main window geometry/state via QSettings.
- Persist separate splitter sizes for strip vs matrix layouts; switching layouts no longer nukes your layout.
- Avoid forcing default window size when saved geometry exists.

## V94 (2026-02-25)

- Added A+E showcase pack (behavior-not-effect + new use-cases): **Boids Swarm**, **Predator / Prey**, **Memory Heatmap**, **Ambient Dashboard**.
- Clarified export status: these showcases are **preview-first** and intentionally **preview-only** for firmware export until their per-target emit paths are audited.
- CI / Release Gate: keeps the `set -euo pipefail` fix so GitHub Actions runs reliably.
