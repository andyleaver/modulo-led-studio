...
## Resource awareness (v1)

Visibility-only performance telemetry and a simple cost heuristic. This does **not** block rendering or export; it only surfaces diagnostics.

- **Files:**
  - `preview/preview_engine.py` (frame/tick/render timing + ops_est + warnings)
  - `qt/qt_app.py` (Health Check: Resource Snapshot)
