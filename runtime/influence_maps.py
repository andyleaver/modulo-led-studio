"""
Influence Maps (engine primitive)
...
from .buffers import ScalarBuffer
...
def deposit_points_scalar(buf: ScalarBuffer,
                             points_xy: Iterable[Tuple[float, float]],
                             cfg: Optional[DepositConfig] = None) -> None:
    """
    Deposit scalar influence into a ScalarBuffer from point samples.
...
def sense_gradient_scalar(buf: ScalarBuffer,
                             x: float,
                             y: float,
                             cfg: Optional[SenseConfig] = None) -> Tuple[float, float]:
    """
...
def steer_follow_gradient(vx: float,
                             vy: float,
                             grad_x: float,
                             grad_y: float,
                             strength: float = 1.0,
