...
def sample_bilinear(self, x: float, y: float) -> float:
    ...
    @staticmethod
    def from_dict(d: Dict[str, Any]) -> "ScalarBuffer":
        w = int(d.get("w", 0))
        h = int(d.get("h", 0))
        cfgd = d.get("cfg", {}) or {}
        buf = ScalarBuffer(BufferConfig(width=w, height=h,
...
class VectorBuffer:
    ...
    @staticmethod
    def from_dict(d: Dict[str, Any]) -> "VectorBuffer":
        w = int(d.get("w", 0))
        h = int(d.get("h", 0))
        cfgd = d.get("cfg", {}) or {}
        buf = VectorBuffer(BufferConfig(width=w, height=h,
                                          clamp_min=float(cfgd.get("clamp_min", 0.0)),
                                          clamp_max=float(cfgd.get("clamp_max", 1.0))))
        vx = d.get("vx", [])
        vy = d.get("vy", [])
