...
_REGISTRY: Dict[str, CAModule] = {}

def list_ca_modules() -> List[str]:
    return sorted(_REGISTRY.keys())

def get_ca_module(name: str) -> Optional[CAModule]:
    return _REGISTRY.get(name)

def register_ca_module(module: CAModule) -> None:
    if not module.name or not isinstance(module.name, str):
        raise ValueError("CAModule.name must be a non-empty string")
    if module.kind not in ("life2d", "elem1d"):
        raise ValueError(f"CAModule.kind must be 'life2d' or 'elem1d' (got {module.kind!r})")
