...
def surface_snapshot(surface: Dict[str, Any], spatial_cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    """
    try:
        from app.project_model import build_surface_from_evidence
        lay = build_surface_from_evidence(surface if isinstance(surface, dict) else {}, default_kind='strip', default_count=1)
    except Exception:
        lay = dict(surface or {}) if isinstance(surface, dict) else {}
    kind, n, mw, mh = get_surface_geometry_values(lay, default_kind='strip', default_count=1)
    ...
    return {
        ...
        "count": n,
        "surface": {
            "width": mw if mw > 0 else None,
            "height": mh if mh > 0 else None,
...
def spatial_snapshot(layout: Dict[str, Any], spatial_cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    return surface_snapshot(layout, spatial_cfg)
