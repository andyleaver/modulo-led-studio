...
def _apply_bounds(ent: _Entity, w: float, h: float, cfg: BoundsConfig) -> None:
    if cfg.mode == "none":
        return
...
def _collide_circle(ent: _Entity, c: CircleObstacle) -> None:
    dx = ent.x - c.x
    dy = ent.y - c.y
    rr = c.r * c.r
    d2 = dx * dx + dy * dy
...
def _collide_segment(ent: _Entity, s: SegmentObstacle) -> None:
...
def _tile_solid(tile: TileMaskObstacle, x: float, y: float) -> bool:
    if not tile.mask:
        return False
...
def _collide_tilemask(ent: _Entity, tile: TileMaskObstacle) -> None:
...
def apply_constraints(
    ...
    *,
    ...
    circles: Optional[Sequence[CircleObstacle]] = None,
    segments: Optional[Sequence[SegmentObstacle]] = None,
    tilemask: Optional[TileMaskObstacle] = None,
) -> None:
    """Apply constraints in-place to entities.

    This is intentionally order-stable and deterministic.
    """
    bcfg = bounds or BoundsConfig(mode="none")
    circles = circles or ()
