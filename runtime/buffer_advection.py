...
from .buffers import ScalarBuffer, VectorBuffer
...
def _sample_scalar_bilinear(buf: ScalarBuffer, fx: float, fy: float, wrap: bool) -> float:
    w, h = buf.w, buf.h
    if w <= 0 or h <= 0:
        return 0.0
...
def advect_scalar_buffer(buf: ScalarBuffer, field: VectorField, cfg: AdvectionConfig) -> None:
...
def advect_vector_buffer(buf: VectorBuffer, field: VectorField, cfg: AdvectionConfig) -> None:
