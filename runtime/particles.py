...
"""
Particle System (engine primitive)
...
Design goals:
- Deterministic when given deterministic dt + seed (TimeSource + project seed)
- Minimal allocations in steady-state
- JSON-serializable state (for World IO / snapshots / replay)
- Surface-agnostic: operates in canonical surface space (x,y floats). Renderers decide mapping.
"""

from .integrators import IntegratorConfig, euler_step_entities
...
def surface_bounds_from_snapshot(surface: Dict[str, Any]) -> Tuple[float, float, float, float]:
    """Return canonical particle bounds from a surface snapshot."""
    ...
    w = surface.get("width") or 1
    h = surface.get("height") or 1
    return (0.0, 0.0, float(w - 1), float(h - 1))


def layout_bounds_from_snapshot(layout: Dict[str, Any]) -> Tuple[float, float, float, float]:
    """Compatibility wrapper for older layout-named callers."""
    return surface_bounds_from_snapshot(layout)
...
class Emitter:
    ...
    def emit(self, system: "ParticleSystem", count: int = 1) -> int:
        spawned = 0
    ...
    def emit_one(self, system: "ParticleSystem") -> Optional[Particle]:
...
ParticleModule = Callable[["ParticleSystem", float, float, Dict[str, float]], None]
...
class ParticleSystem:
    ...
    @staticmethod
    def from_dict(d: Dict[str, Any]) -> "ParticleSystem":
        sys = ParticleSystem(seed=int(d.get("seed", 0)), max_particles=int(d.get("max_particles", 2000)))
    ...
    # ---- bounds helpers
    def set_bounds_from_surface(self, surface: Dict[str, Any]) -> None:
        """Set particle bounds from canonical surface geometry."""
        self.bounds = surface_bounds_from_snapshot(surface)

    def set_bounds_from_layout(self, layout: Dict[str, Any]) -> None:
        return self.set_bounds_from_surface(layout)
...
def module_constant_gravity(gx: float = 0.0, gy: float = 0.0) -> ParticleModule:
    ...
    def _mod(sys: ParticleSystem, dt: float, t: float, signals: Dict[str, float]) -> None:
...
def module_field_advection(field, strength: float = 1.0) -> ParticleModule:
    ...
    def _mod(sys: ParticleSystem, dt: float, t: float) -> None:
