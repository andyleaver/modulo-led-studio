...
Typical usage inside a behavior:
    ...
    from runtime.fsm import FSM, State, Transition, step_fsm
    ...
    def init_state():
        fsm = FSM(
            ...
            start='intro',
            transitions=[
                Transition(src='intro', dst='run', after_s=2.0),
                Transition(src='run', dst='win', event='goal'),
            ],
        )
        return {'fsm': fsm}

    def update(..., state):
        step_fsm(state['fsm'], dt=dt, now=t, vars=vars, signals=signals, events=events)
...
class Transition:
    ...
    on_transition: Optional[Callable[["FSM", str, str, Dict[str, Any]], None]] = None
...
def make_phase_fsm(phases: Sequence[Tuple[str, float]], loop: bool = True) -> FSM:
    ...
    states = {name: State(name) for name, _dur in phases}
    transitions: List[Transition] = []
    for i, (name, dur) in enumerate(phases):
        if i < len(phases) - 1:
            transitions.append(Transition(src=name, dst=phases[i + 1][0], after_s=float(dur)))
        else:
            if loop and len(phases) > 0:
                transitions.append(Transition(src=name, dst=phases[0][0], after_s=float(dur)))
    start = phases[0][0] if phases else ""
    return FSM(states=states, start=start, transitions=transitions)
