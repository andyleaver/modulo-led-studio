...
def make_layerstack_sketch(*, project: dict) -> str:
    ...
    for L in layers:
        ...
        if beh == 'red_hat_runner':
            ...
            from runtime.ca_modules_v1 import list_ca_modules, get_ca_module
            params = dict(params)
        ...
        # Phase 3.6: write_the_loop C++ body per layer (injected at export time).
        if beh in ("write_the_loop", "fsm_phases"):
...
""".strip()
            if "#" in cpp_body:
                raise ExportValidationError("[E_WRITE_LOOP_INVALID] write_the_loop cpp body may not contain preprocessor directives (#).")
...
static inline void evalKernelDSL(int li, int i, float t, float br, uint32_t seed_u32, float &r, float &g, float &b) {
  // Normalized coords.
#ifdef MATRIX_WIDTH
  const float x = (MATRIX_WIDTH <= 1) ? 0.0f : ((float)(i % MATRIX_WIDTH) / (float)(MATRIX_WIDTH - 1));
  const float y = (MATRIX_HEIGHT <= 1) ? 0.0f : ((float)(i / MATRIX_WIDTH) / (float)(MATRIX_HEIGHT - 1));
#else
...
def export_project_layerstack(*, project: dict, template_path, out_path, replacements: dict | None = None):
    ...
    # Optional low-level target hooks (includes/defines/setup/loop injections)
    _hooks = _load_target_hooks(Path(template_path) if template_path is not None else None)
    code = _inject_target_hooks(code, _hooks)
...
# Wiring defaults (UI may override these; otherwise fall back to project layout)
    ...
    # Matrix defaults: ensure required matrix tokens are present when a matrix/cells layout is in use.
    # Older projects store matrix dimensions in layout.matrix_w/matrix_h (and keep layout.type='strip').
    try:
        shape = str(layout.get('shape') or layout.get('kind') or '').strip().lower()
        mw_hint = layout.get('matrix_w') or layout.get('mw') or layout.get('width') or 0
        mh_hint = layout.get('matrix_h') or layout.get('mh') or layout.get('height') or 0
        is_matrixish = (shape in ('matrix', 'cells')) or (int(mw_hint or 0) > 1 and int(mh_hint or 0) > 1)
        if is_matrixish:
            exp0 = (project or {}).get('export') or {}
            hw0 = exp0.get('hw') or exp0.get('hardware') or {}
            mcfg = hw0.get('matrix') if isinstance(hw0, dict) else None
            if isinstance(mcfg, dict):
                mw = int(mcfg.get('width') or mw_hint or 0)
                mh = int(mcfg.get('height') or mh_hint or 0)
                serp = mcfg.get('serpentine')
                origin = str(mcfg.get('origin') or layout.get('origin') or 'top_left').strip()
                rot_src = mcfg.get('rotate')
                fx_src = mcfg.get('flip_x')
                fy_src = mcfg.get('flip_y')
            else:
                mw = int(mw_hint or 0)
                mh = int(mh_hint or 0)
                serp = layout.get('matrix_serpentine') if 'matrix_serpentine' in layout else layout.get('serpentine')
                origin = str(layout.get('origin') or 'top_left').strip()
                rot_src = layout.get('matrix_rotate') if 'matrix_rotate' in layout else layout.get('rotate')
                fx_src = layout.get('matrix_flip_x') if 'matrix_flip_x' in layout else layout.get('flip_x')
                fy_src = layout.get('matrix_flip_y') if 'matrix_flip_y' in layout else layout.get('flip_y')

            if mw > 0 and mh > 0:
                replacements.setdefault('MATRIX_WIDTH', str(mw))
                replacements.setdefault('MATRIX_HEIGHT', str(mh))
                # For matrix layouts, LED count is derived from width*height (but do not override if project explicitly uses a different count).
                try:
                    existing_n = int(layout.get('led_count') or layout.get('num_leds') or layout.get('count') or 0)
                except Exception:
                    existing_n = 0
                calc_n = int(mw) * int(mh)
                if existing_n in (0, calc_n):
                    layout['led_count'] = calc_n
                    layout['num_leds'] = calc_n
            ...
            fy = 0
            try:
                fx = 1 if bool(fx_src or False) else 0
                fy = 1 if bool(fy_src or False) else 0
            except Exception:
                fx = 0
                fy = 0
