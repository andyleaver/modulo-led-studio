...
def _diag_exc(e: Exception, where: str):
    try:
        if _DIAGS is not None:
            _DIAGS.exception(e, domain="EXPORT", code="TEENSY_EMITTER_EXCEPTION", summary=where)
    except Exception:
        pass
...
def emit(*, ir: ShowIR, out_path: Path, **_kwargs) -> Tuple[Path, str]:
    """Teensy 4.1 (FastLED, no audio) target pack (scaffold, audio disabled)."""
    ...
    report = (
        "Target: teensy41_fastled_noneaudio\n"
