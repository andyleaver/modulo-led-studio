...
def emit(*, ir: ShowIR, out_path: Path, **_kwargs) -> Tuple[Path, str]:
    """ESP32 HUB75 (I2S-DMA) matrix target pack (no-audio)."""
    tpl = Path(__file__).resolve().parent / "arduino_template.ino.tpl"
    meta = _get_meta()

    # Matrix dimensions are taken from export.hw.matrix when present, otherwise layout width/height.
    # (export_project_validated already resolves MATRIX_* tokens)
    ...
    # Basic hw values (used for report and for any incidental tokens)
    ...
    replacements = {
        ...
        # Keep these around for templates expecting them; they are unused for HUB75.
    ...
    report = (
        f"Target: {meta.get('id','esp32_hub75_i2sdma_noneaudio')}\n"
        f"Written: {p}\n"
