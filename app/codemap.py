...
def _locate_assignment(module_name: str, var_name: str) -> Optional[CodeLocation]:
    ...
    Used for registries like REGISTRY/TARGETS which are dicts (inspect can't locate them).
    """
    try:
        import importlib
        ...
        # Match 'VAR =' at beginning of a line (allow whitespace)
        pat = re.compile(r"^\s*" + re.escape(var_name) + r"\s*=")
        for i, line in enumerate(lines, start=1):
            if pat.search(line):
                return CodeLocation(file=str(p), start_line=i, end_line=i)
        return None
    except Exception:
        return None
...
def build_codemap() -> Dict[str, Optional[CodeLocation]]:
    ...
    items: List[Tuple[str, str]] = [
        ...
        ("eras.state.ensure_era_in_project", "app.eras.era_state:ensure_era_in_project"),
        ...
        ("behaviors.registry.list_effect_keys", "behaviors.registry:list_effect_keys"),
    ...
    for label, spec in items:
        try:
            ...
            if out[label] is None:
                last = sym.split('.')[-1]
                if last.isupper() and '.' not in last:
                    out[label] = _locate_assignment(mod_name, last)
        except Exception:
            out[label] = None
