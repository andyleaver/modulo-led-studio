...
from preview.time_source_v1 import TimeSourceV1
from runtime.spatial_v1 import spatial_snapshot
from runtime.extensions_v1 import collect_signal_overrides, collect_system_registrations
...
def _call_preview_emit(beh, *, num_leds: int, params: dict, t: float, state: dict, layout: dict, dt: float, audio: dict):
    ...
    if res is None:
        # ---- perf: capture render + frame timing + snapshot ----
        try:
            if _render_t0 is not None:
                _render_ms = (_time_mod.perf_counter() - _render_t0) * 1000.0
                try:
                    self._perf_render_ms.append(float(_render_ms))
                except Exception:
                    pass
        except Exception:
            _render_ms = None
        ...
        try:
            if _frame_t0 is not None:
                _frame_ms = (_time_mod.perf_counter() - _frame_t0) * 1000.0
                try:
                    self._perf_frame_ms.append(float(_frame_ms))
                except Exception:
                    pass
        except Exception:
            _frame_ms = None

        # resource snapshot (visibility only)
        try:
            _en_layers = 0
            for _L in layers:
                if bool(_lg(_L, 'enabled', True)):
                    _en_layers += 1
            _ops_est = int(n) * int(max(_en_layers, 1))
            _warnings = []
            if _ops_est > 200000:
                _warnings.append('heavy_pixels_x_layers')
            # detect O(n^2)-ish agent workloads (heuristic)
            if any('boid' in (str(_lg(_L,'behavior',_lg(_L,'effect',''))).lower()) for _L in layers):
                _warnings.append('possible_agent_neighbor_cost')
            self._resource_snapshot = {
                'n_leds': int(n),
                'enabled_layers': int(_en_layers),
                'ops_est': int(_ops_est),
                'tick_ms_avg': float(sum(self._perf_tick_ms)/len(self._perf_tick_ms)) if getattr(self,'_perf_tick_ms',None) and len(self._perf_tick_ms)>0 else None,
                'render_ms_avg': float(sum(self._perf_render_ms)/len(self._perf_render_ms)) if getattr(self,'_perf_render_ms',None) and len(self._perf_render_ms)>0 else None,
                'frame_ms_avg': float(sum(self._perf_frame_ms)/len(self._perf_frame_ms)) if getattr(self,'_perf_frame_ms',None) and len(self._perf_frame_ms)>0 else None,
                'last_layer_costs': list(_layer_costs[-8:]) if isinstance(_layer_costs, list) else [],
                'warnings': _warnings,
            }
            try:
                self._perf_layers_ms.append(list(_layer_costs[-8:]))
            except Exception:
                pass
        except Exception:
            pass
...
def _normalize_modulotors(mods):
    ...
    try:
        prev = getattr(self, "_prev_frame_rgb", None)
        occ_thr = int(getattr(self, "_occ_thr", 8))
        npx = int(len(out) if isinstance(out, list) else 0) or 0
        entropy = 0.0
        occupancy = 0.0
        if npx > 0:
            # Occupancy: fraction of pixels with mean channel > threshold
            try:
                on = 0
                for (r,g,b) in out:
                    if ((int(r)+int(g)+int(b))//3) > occ_thr:
                        on += 1
                occupancy = float(on) / float(npx)
            except Exception:
                occupancy = 0.0
            # Entropy: normalized mean absolute delta vs previous frame
            if isinstance(prev, list) and len(prev) == npx:
                try:
                    acc = 0.0
                    for (r0,g0,b0),(r1,g1,b1) in zip(prev, out):
                        acc += abs(int(r1)-int(r0)) + abs(int(g1)-int(g0)) + abs(int(b1)-int(b0))
                    entropy = float(acc) / float(npx * 255 * 3)
                except Exception:
                    entropy = 0.0
        # Activity: EMA of entropy
        a_prev = float(getattr(self, "_activity_ema", 0.0) or 0.0)
        alpha = float(getattr(self, "_activity_alpha", 0.10) or 0.10)
        if alpha < 0.0: alpha = 0.0
        if alpha > 1.0: alpha = 1.0
        activity = (a_prev * (1.0 - alpha)) + (entropy * alpha)
        self._activity_ema = float(activity)
        # Motion: prefer agent/system motion if available, else activity
        motion = float(getattr(self, "_motion_est", activity) or activity)
        # Clamp 0..1
        def _c01(x):
            return 0.0 if x < 0.0 else (1.0 if x > 1.0 else x)
        derived = {
            "signal.entropy": _c01(float(entropy)),
            "signal.activity": _c01(float(activity)),
            "signal.occupancy": _c01(float(occupancy)),
            "signal.motion": _c01(float(motion)),
        }
        # Allow extensions to contribute derived signals (engine-only, safe).
        try:
            ctx = {"project": getattr(self, "project", None), "layout": layout, "t": float(t), "dt": float(dt), "frame": int(getattr(self, "_frame", 0))}
            ext = collect_signal_overrides(ctx)
            if isinstance(ext, dict) and ext:
                derived.update(ext)
        except Exception:
            pass

        # Publish to SignalBus (best-effort). Keep audio/vars unchanged.
        try:
            if self.signal_bus is not None and hasattr(self.signal_bus, "update"):
                self.signal_bus.update(
                    t=float(getattr(self.time_source._clock, "sim_time", 0.0)),
                    dt=float(getattr(self.time_source, "fixed_dt", 1.0/60.0)),
                    frame=int(getattr(self, "_frame_index", 0)),
                    audio_state=getattr(self, "last_audio_state", None),
                    variables_state=getattr(self, "_last_vars_state", None),
                    time_mode=str(getattr(self.time_source, "mode", "SIM_FIXED_DT")),
                    time_paused=bool(getattr(self.time_source, "paused", False)),
                    time_tick=int(getattr(getattr(self.time_source, "_clock", None), "tick", 0)),
                    time_fixed_dt=float(getattr(self.time_source, "fixed_dt", 1.0/60.0)),
        ...
        try:
            self._prev_frame_rgb = list(out) if isinstance(out, list) else None
        except Exception:
...
class PreviewEngine:
    ...
    def __init__(self, project, audio, fixed_dt: float = 1.0 / 60.0, signal_bus=None):
        ...
        self.time_source = TimeSourceV1(mode=str((_pd or {}).get('time_v1', {}).get('mode', 'SIM_FIXED_DT')), fixed_dt=float(fixed_dt), seed=int((_pd or {}).get('time_v1', {}).get('seed', 1) or 1))
        self.systems = SystemSchedulerV1()
        # Default anchor nodes to allow stable ordering
        self.systems.register("time", lambda ctx: None)
        self.systems.register("render", lambda ctx: None, after={"time"})
        # Register extension-provided engine systems
        try:
            for _name, _fn, _after, _before, _en in collect_system_registrations():
                if _name in ("time", "render"):
                    continue
                self.systems.register(_name, _fn, after=set(_after or set()), before=set(_before or set()), enabled=bool(_en))
        except Exception:
            pass
        # ---- Resource awareness v1 (non-blocking telemetry; visibility only) ----
        try:
            from collections import deque
            self._perf_frame_ms = deque(maxlen=240)
            self._perf_tick_ms = deque(maxlen=240)
            self._perf_render_ms = deque(maxlen=240)
            self._perf_layers_ms = deque(maxlen=120)  # list of (behavior, ms) tuples
        except Exception:
            self._perf_frame_ms = []
            self._perf_tick_ms = []
            self._perf_render_ms = []
            self._perf_layers_ms = []
        self._resource_snapshot = {}
        try:
            _tv1 = (_pd or {}).get('time_v1', {}) or {}
            self.time_source.set_fixed_dt(float(_tv1.get('fixed_dt', fixed_dt)))
            self.time_source.set_paused(bool(_tv1.get('paused', False)))
        except Exception:
            pass
        self._frame_index = 0
        ...
        # Register for performance health probe
        try:
            from preview.engine_registry import LAST_PREVIEW_ENGINE
            import preview.engine_registry as _reg
            _reg.LAST_PREVIEW_ENGINE = self
        except Exception:
            pass
    def render_frame(self, t: float) -> List[RGB]:

        try:
            # Advance canonical time (driven by incoming wall timestamp t)
            snap = self.time_source.step_wall(float(t))
            _prev_sim = float(getattr(self, '_last_sim_t', 0.0))
            steps = int(getattr(snap, 'tick', 0) - int(getattr(self, '_last_tick', 0)))
            _dt = float(getattr(snap, 'dt', 1.0/60.0))
            self._last_tick = int(getattr(snap, 'tick', 0))
            self._last_sim_t = float(getattr(snap, 't', 0.0))
            self._frame_index = int(getattr(snap, 'frame', 0))
            ...
            try:
                ...
                _frame_t0 = _time_mod.perf_counter()
            except Exception:
                _frame_t0 = None
            ...
            try:
                if getattr(self, 'audio', None) is not None and hasattr(self.audio, 'step'):
                    ...
                    try:
                        _t = float(getattr(self, "_last_sim_t", 0.0))
                    except Exception:
                        _t = 0.0
            ...
            try:
                self._last_vars_state = dict(_clock_vars) if isinstance(_clock_vars, dict) else {}
            except Exception:
            ...
            # ---- SystemSchedulerV1 tick (engine primitives) ----
            try:
                ctx = {
                    ...
                    "shape": shape,
                    "num_leds": int(n),
                    "t": float(getattr(snap, "t", 0.0)),
                    "dt": float(getattr(snap, "dt", _dt)),
                    "tick": int(getattr(snap, "tick", 0)),
                    "frame": int(getattr(snap, "frame", 0)),
                    "signal_bus": self.signal_bus,
                    "audio": self.audio,
                }
                self.systems.tick(ctx)
                self._systems_snapshot = self.systems.snapshot()
                self._systems_errors = ctx.get("_system_errors", [])
            except Exception:
                self._systems_snapshot = {"systems": []}
                self._systems_errors = []
...
# Ensure per-layer persistent state exists (engine-owned, keyed by stable layer uid)
            ...
            # ---- perf: tick/update timing ----
            try:
                import time as _time_mod
                _tick_t0 = _time_mod.perf_counter()
            except Exception:
                _tick_t0 = None

            # Fixed-tick updates for stateful/game layers (deterministic)
            if steps > 0:
                _dt = float(getattr(self.time_source, 'fixed_dt', 1.0/60.0))
            ...
            # ---- perf: capture tick/update timing ----
            try:
                if _tick_t0 is not None:
                    _tick_ms = ( _time_mod.perf_counter() - _tick_t0 ) * 1000.0
                    try:
                        self._perf_tick_ms.append(float(_tick_ms))
                    except Exception:
                        pass
            except Exception:
                pass
            ...
            # ---- perf: render timing ----
            try:
                _render_t0 = _time_mod.perf_counter()
                _layer_costs = []
            except Exception:
                _render_t0 = None
                _layer_costs = []
            ...
            for _li, L in enumerate(layers):
                ...
                for k,v in (self._purpose or {}).items():
                    if k.startswith('purpose_'):
                        audio_ctx[k]=v
                sim_t = float(getattr(self.time_source._clock, 'sim_time', t))
                params = resolve(base_params, sim_t, audio=audio_ctx, modulotors=mods)
                # : provide layer states + index for cross-layer actions
                try:
                    params['_all_states'] = list(all_states)
                ...
                try:
                    ...
                    pass
                try:
                    # ---- perf: per-layer render cost ----
                    try:
                        _lt0 = _time_mod.perf_counter()
                    except Exception:
                        _lt0 = None
                ...
                # ---- perf: collect per-layer render cost ----
                try:
                    if _render_t0 is not None and _lt0 is not None:
                        _ms = (_time_mod.perf_counter() - _lt0) * 1000.0
                        _layer_costs.append((str(getattr(beh, 'name', key)), float(_ms)))
                except Exception:
                    pass
            ...
            try:
                ...
                # ---- Spatial semantics snapshot (v1; data-only) ----
                try:
                    _sp = spatial_snapshot(layout, (self.project or {}).get('spatial_v1'))
                    layout['space'] = dict(_sp)
                    self._spatial_snapshot = dict(_sp)
                except Exception:
                    pass
