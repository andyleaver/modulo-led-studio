...
def _preview_emit(*, num_leds: int, params: dict, t: float) -> List[RGB]:
    ...
    softness = _clamp01(float(params.get("softness", 0.55))) # blend curve
    density = _clamp01(float(params.get("density", 0.5)))    # number of mirrors/slices

    if width < 0.05: width = 0.05
    slices = 2 + int(round(density * 10.0))  # 2..12
    phase = t * max(0.1, speed) * 0.25
    ...
    for i in range(n):
        ...
        # mirror in slices
        u = (x * slices) % 1.0
        u = min(u, 1.0 - u) * 2.0  # 0..1 mirrored
        # value field
        v = 0.5 + 0.5*math.sin(2.0*math.pi*(u/width + phase))
        v = pow(_clamp01(v), exp)
        hue = (u + phase*0.35 + v*0.25) % 1.0
        r,g,b = _hsv_to_rgb(hue, 1.0, br * (0.25 + 0.75*v))
        out.append((r,g,b))
    return out
...
def _arduino_emit(*, layout: dict, params: dict) -> str:
    ...
    slices = 2 + int(round(density * 10.0))
    exp = 0.7 + (1.0-softness)*2.0
    period_ms = int(max(800.0, min(60000.0, 20000.0 / max(0.2, speed))))

    return f"""// Generated by Modulo (Effect: kaleidoscope)
#include <FastLED.h>
#include <math.h>
...
void loop() {{
  ...
  for (int i=0; i<NUM_LEDS; i++) {{
    ...
    float u = fmodf(x * (float){slices}f, 1.0f);
    u = fminf(u, 1.0f - u) * 2.0f;

    float v = 0.5f + 0.5f*sinf(2.0f*3.1415926f*(u/width + phase*0.25f));
    v = powf(clamp01(v), exp);
    float hue = fmodf(u + phase*0.35f + v*0.25f, 1.0f);

    uint8_t r,g,b;
    hsv_to_rgb(hue, 1.0f, br * (0.25f + 0.75f*v), &r, &g, &b);
    leds[i] = CRGB(r,g,b);
  }}
