...
from runtime.force_particles_core_v1 import integrate_point_forces_v1
from runtime.particle_pairs_v1 import count_pairs_within_radius_v1
...
def _update(*, state: EffectState, params: dict, dt: float, t: float, audio: dict | None = None) -> None:
    ...
    # Integrate (point forces) via lifted core primitive.
    max_v = 25.0
    point_forces = [f for f in forces if isinstance(f, dict) and str(f.get('kind','point')) in ('point','force','attract','repel')]

    def _num(name: str, default: float = 1.0) -> float:
        return float(_get_number_var_ref(params, state, name, default))

    def _tog(name: str, default: bool = False) -> bool:
        return bool(_get_toggle_var_ref(params, state, name, default))

    killed, wrap_count, wall_hit_count, bounds_exit_count = integrate_point_forces_v1(
        parts=parts,
        ...
        h=int(h),
        spd=float(spd),
        cx_center=float(cx_center),
        cy_center=float(cy_center),
        max_v=float(max_v),
    )
...
# : remove killed particles (bounds destroy mode)
    ...
    pp_near, pp_coll = count_pairs_within_radius_v1(
        parts=parts,
        radius=float(pp_r),
        max_pairs_checked=int(pp_max),
        mode=str(pp_mode),
    )
...
def _preview_emit(*, num_leds: int, params: dict, t: float, state: EffectState | None = None) -> List[RGB]:
    ...
    This is intentionally thin: simulation lives in core primitives.
    Rendering uses the engine primitive ParticleRender v1 so mapping is shared.
    """
    from runtime.particle_render_v1 import render_points_to_leds_v1, ParticleRenderConfigV1
    ...
    # Build a minimal layout dict for ParticleRender v1.
    coords = params.get("_coords")
    if not (isinstance(coords, list) and len(coords) == n):
        coords = [(int(i % w), int(i // w)) for i in range(n)] if h > 1 else [(int(i), 0) for i in range(n)]
    layout = {"num_leds": n, "coords": coords}
    ...
    if pts:
        render_points_to_leds_v1(
            ...
            points_xy=pts,
            colors=cols,
            config=ParticleRenderConfigV1(radius=0.85, additive=True),
        )

    # Target dots (rules) rendered via same primitive
    targets_cfg = _get_targets_from_params(params)
    if targets_cfg:
        ...
        for i, cfg in enumerate(targets_cfg):
            ...
            if dx is None or dy is None:
                continue
            try:
                dot_pts.append((float(dx), float(dy)))
                dc = cfg.get("color", (0, 255, 0))
                dot_cols.append(_apply_brightness((int(dc[0]) & 255, int(dc[1]) & 255, int(dc[2]) & 255), br))
            except Exception:
                continue
        ...
        if dot_pts:
            render_points_to_leds_v1(
                ...
                points_xy=dot_pts,
                colors=dot_cols,
                config=ParticleRenderConfigV1(radius=0.55, additive=True),
            )
